...
Любая информация (текст, html&#8209;теги, веб&#8209;ссылки), введенная вами в блоке «Narrative&nbsp;content» для стоп&#8209;кадра, 
...
вы легко можете используя представленные ниже шаблоны (html&#8209;заготовки), адаптировав (отредактировав) их для вашего конкретного случая и в соответствии с вашими вкусовыми предпочтениями 
...
Применяйте шаблон №&nbsp;1 **только для оформления дополнительных стоп&#8209;кадров**, если решите дополнить ими вашу анимацию *(что, конечно, приветствуется)* – то есть только тогда, 
когда вы уже создали оформление для как минимум одного стоп&#8209;кадра в соответствии с более сложными шаблонами №&nbsp;2 и/или №&nbsp;3, включающими html&#8209;ссылки на внешние веб&#8209;ресурсы.

*Пример простого оформления стоп&#8209;кадра:*

![](../media/2024-03-29_14-47-17.png) \
 \
*html-код простого оформления стоп&#8209;кадра:*
```html
...
```
* *Тег* **`<!-- -->`** *добавляет в код комментарий (синтаксис* **`<!-- текст комментария -->`***). \
  Комментарии помогают сделать код более понятным. Они позволяют добавлять пояснения и заметки, которые не показываются в браузере и, соответственно, не&nbsp;будут влиять на отображение веб&#8209;страницы. 
  Комментарии полезны как для объяснения сложных частей кода, так и временного отключения (вместо удаления) частей кода при отладке или тестировании различных его версий.*
...
*Пример оформления стоп&#8209;кадра с веб&#8209;ссылкой:*
...
```
* *Атрибут* **`target="_blank"`** *определяет, что ссылка (в данном случае на страницу Википедии) будет открываться в новой вкладке браузера.*
...
*Пример более сложного оформления стоп&#8209;кадра с веб&#8209;ссылкой:*
